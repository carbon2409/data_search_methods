...
while page_superjob <= 3:
    ...
    for vac_job in vacancies_superjob:
        ...
        vacs.append(vac_data_job)
        cycle_count_job += 1

    page_superjob += 1

#Создаем базу данных вакансий
from pymongo import MongoClient

client = MongoClient('localhost', 27017)
db = client['vacancies_database']
vacancies_ = db.vacancies
vacancies_.insert_many(vacs)
objects = vacancies_.find({'min_pay':{'$gte':int(input('Введите минимальную зарплату '))}})
for obj in objects:
    print(obj)
